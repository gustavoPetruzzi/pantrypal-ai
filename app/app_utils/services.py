...
def get_memory_service():
    ...
    if uri := os.environ.get("MEMORY_SERVICE_URI"):
        if uri.startswith("agentengine://"):
            bank_id = uri.replace("agentengine://", "")
            from google.adk.memory import VertexAiMemoryBankService

            return VertexAiMemoryBankService(
                project=os.environ.get("GOOGLE_CLOUD_PROJECT", "qwiklabs-gcp-01-b551540f472b"),
    ...
    if memory_bank_id := os.environ.get("GOOGLE_CLOUD_MEMORY_BANK_ID", "4171410226584485888"):
        from google.adk.memory import VertexAiMemoryBankService
